...
class NotifySubscriptionRenewalFailure extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    {
        $title = 'Customer Charge Failed for Recurring Payment Plan '.$this->recurringPlan->name;
        $title = App::environment('production') ? $title : '['.App::environment().'] '.$title;

        return (new MailMessage)->view('hitpay-email.recurring.card-failed', [
            'title' => $title,
            ...
            'plan_payment_provider' => $this->recurringPlan->payment_provider,
