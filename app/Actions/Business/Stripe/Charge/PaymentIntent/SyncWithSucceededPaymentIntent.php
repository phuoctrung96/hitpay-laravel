...
class SyncWithSucceededPaymentIntent extends Action
{
    ...
    public function process() : ?Business\PaymentIntent
    {
        ...
        if ($stripePaymentIntent->status !== 'succeeded') {
            $expectedBusinessPaymentIntentStatus = Stripe\PaymentIntent::STATUS_SUCCEEDED;

            throw new Exceptions\BadRequest("The status of the Stripe payment intent (ID : {$stripePaymentIntent->id}) is currently `{$stripePaymentIntent->status}`, it must be in status `{$expectedBusinessPaymentIntentStatus}` to continue.");
        }

        if ($this->businessPaymentIntent->status === Stripe\PaymentIntent::STATUS_SUCCEEDED) {
            throw new Exceptions\BadRequest("The status of the business payment intent (ID : {$this->businessPaymentIntent->id}) is currently `{$this->businessPaymentIntent->status}`, which is unable to continue.");
        }
