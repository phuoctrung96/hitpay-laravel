...
class SendExportedPlatformCharges implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $charges = $this->business->platformCharges()->with('receiptRecipients', 'target')->whereIn('status', [
        ...
        foreach ($charges as $charge) {
            ...
            $singleData = collect([
                ...
                'Commission' => getReadableAmountByCurrency($charge->currency, $charge->commission_amount),
        ...
        if ($this->user instanceof User) {
            $this->user->notify(new SendFile($this->business->getName().' - Exported Platform Charges', [
                'Please find attached the exported platform charges'.$filters.' from '.$fromDate.' to '.$toDate,
            ], ($fromDate.' - '.$toDate), $csv->getContent()));
        } else {
            $this->business->notify(new SendFile('Your Exported Platform Charges', [
                'Please find attached your exported platform charges'.$filters.' from '.$fromDate.' to '.$toDate,
            ], ($fromDate.' - '.$toDate), $csv->getContent()));
        }
        ...
        foreach ($businessAdmins as $businessAdmin) {
            $businessAdmin->user->notify(new SendFile($this->business->getName().' - Exported Platform Charges', [
                'Please find attached the exported platform charges'.$filters.' from '.$fromDate.' to '.$toDate,
            ], ($fromDate.' - '.$toDate), $csv->getContent()));
        }
    }
