<template>
    <div>
        <button class="btn btn-primary" data-toggle="modal" :data-target="'#'+type+'ExportModal'">Export {{type}} <template v-if="type==='campaigns'">cashbacks</template></button>
        <div class="modal fade" :id="type+'ExportModal'" tabindex="-1" role="dialog" aria-labelledby="refundExportModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="refundExportModalLabel">Export</h5>
                    ...
                    <div v-else class="modal-body">
                        ...
                        <div class="text-right">
                            <button id="downloadBtn" type="button" class="btn btn-primary" @click.prevent="requestReport" :disabled="is_requesting">
                                Export {{type}} <template v-if="type==='campaigns'">cashbacks</template> <i v-if="is_requesting" class="fas fa-spinner fa-spin"></i>
                            </button>
                        </div>
                    </div>
                </div>
    ...
    export default {
        ...
        methods: {
            requestReport() {
                ...
                if (Business !== null) {
                    url = this.getDomain('business/' + Business.id + '/refund/export', 'admin');
                } else {
                    url = this.getDomain('refund/export', 'admin')
                }
