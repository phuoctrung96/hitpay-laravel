        ...
        <div class="content__input w-50">
            <div class="content__input-wrapper">
                ...
                <div>
                    ...
                    <input type="text" v-model="template.email_subject" class="form-control medium  p-2" :placeholder="`Your Receipt from {{business_name}}`"/>
                </div>
                <div class="mt-3">
                    <h6 class="content__input-label small">Title</h6>
                    <input type="text" v-model="template.title" class="form-control medium  p-2" :placeholder="`{{business_name}}`"/>
                </div>
                <div class="mt-3">
                    <h6 class="content__input-label small">Subtitle</h6>
                    <input type="text" v-model="template.subtitle" class="form-control medium  p-2" placeholder="View Transaction details below"/>
                </div>
                <div class="mt-4">
                    <h6 class="content__input-label small text-secondary">STORE INFORMATION</h6>
                </div>
                <div class="mt-3">
                    <h6 class="content__input-label small">Title</h6>
                    <input type="text" value="Seller information" v-model="template.store_information_title" class="form-control medium  p-2" :placeholder="`Seller information`"/>
                </div>
                <div class="mt-3">
                    <h6 class="content__input-label small">Value</h6>
                    <input type="text" v-model="template.store_information_value" class="form-control medium  p-2" :placeholder="`{{business_name}} ({{business_email}})`"/>
                </div>
                <div class="mt-3">
        ...
        <div class="content__preview w-50">
            ...
            <div class="content__preview-card mt-2">
                ...
                <div class="mt-3">
                    ...
                    <div class="mt-4">
                        <h6 class="small text-secondary">Order ID</h6>
                        <h6 class="small">9499-c656449778999332aa291</h6>
                    </div>
                    <div class="mt-4">
                        <h6 class="small text-secondary">{{previewSellerTitle}}</h6>
                        <h6 class="small">{{previewSellerInfo}}</h6>
                    </div>

                    <div class="description description--order"> 
                        <ul class="d-flex description__header flex-row justify-content-between">
                            <li><small class="text-secondary">Item</small></li>
                            <li><small class="text-secondary">Amount</small></li>
                        </ul>
                        <div class="description__content">
                            <ul class="d-flex mt-3 sub-header flex-row justify-content-between">
                                <li style="color: #0A1734">Apple airpods x1</li>
                                <li>SGD 150.00</li>
                            </ul>
                            <ul class="d-flex sub-item flex-row justify-content-between mt-2">
                                <li class="text-secondary">Shipping method: Sipping</li>
                                <li class="text-secondary">SGD 0.00</li>
                            </ul>
                            <ul class="d-flex sub-item flex-row justify-content-between mt-2">
                                <li class="text-secondary">Discount</li>
                                <li class="text-secondary">- SGD 2.00</li>
                            </ul>
                            <ul class="d-flex sub-item flex-row justify-content-between mt-2">
                                <li class="text-secondary">Coupon Discount</li>
                                <li class="text-secondary">- SGD 2.00</li>
                            </ul>
...
export default {
    ...
    methods: {
        ...
        reset() {
            ...
            axios.put(this.getDomain(url, "api"), {
                ...
                const { order_confirmation_template } = s.data;
                this.template =order_confirmation_template;
                this.template['template_for'] = 'order_confirmation_template'
        ...
        async fetchTemplates() {
            ...
            .then(s => {
                const { order_confirmation_template } = s.data;
                order_confirmation_template['seller_info'] = this.businessFormatted.name + ' (' + this.businessFormatted.email + ')'
                this.template =order_confirmation_template;
                this.template['template_for'] = 'order_confirmation_template'
            })
            .catch(err => {
    ...
    computed: {
        ...
        previewSellerInfo() {
            if(this.template?.store_information_value) {
                return this.parseContentWithVariables(this.template?.store_information_value)
            }
            return this.businessFormatted.name + ' (' + this.businessFormatted.email + ')'
        },
        previewSellerTitle() {
            if(this.template?.store_information_title) {
                return this.parseContentWithVariables(this.template?.store_information_title)
            }
            return "Seller Info"
        },
        previewTitle() {
