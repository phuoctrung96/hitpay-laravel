<template>
    <div class="p-4">
        <button class="btn btn-primary" data-toggle="modal" data-target="#exportModal">Export Orders</button>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div v-else class="modal-body">
                        <select v-model="statusForm" class="form-control form-control-sm">
                            <option v-for="(value,name) in statuses" :value="value" :key="value">
                                {{ name }}
                            </option>
                        </select>
                        <div>
                            <h5 class="mt-4">Date of creation</h5>
                            <div class="form-group">
                                <label class="col-form-label">From:</label>
                                <div class="input-group">
                                    <select v-model="date.from.year" aria-label="Year" class="custom-select"
                                            :disabled="is_requesting">
                                        <option v-for="year in form.years" :value="year">{{ year }}</option>
                                    </select>
                                    <select v-model="date.from.month" aria-label="Year" class="custom-select"
                                            :disabled="is_requesting">
                                    ...
                                    <select v-model="date.from.day" aria-label="Year" class="custom-select"
                                            :disabled="is_requesting">
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <select v-model="date.to.year" aria-label="Year" class="custom-select"
                                            :disabled="is_requesting">
                                        <option v-for="year in form.years" :value="year">{{ year }}</option>
                                    </select>
                                    <select v-model="date.to.month" aria-label="Year" class="custom-select"
                                            :disabled="is_requesting">
                                        <option v-for="(month, index) in form.months" :value="index">{{
                                                month
                                            }}
                                    ...
                                    <select v-model="date.to.day" aria-label="Year" class="custom-select"
                                            :disabled="is_requesting">
                                        <option v-for="day in form.days" :value="day">{{ day }}</option>
                                    </select>
                                </div>
    ...
    methods: {
        requestReport() {
            ...
            if (submissionData.starts_at > submissionData.ends_at) {
                ...
                axios.post(this.getDomain('business/' + Business.id + '/order/export', 'dashboard'), submissionData).then(({data}) => {
                    this.is_requesting = false;
