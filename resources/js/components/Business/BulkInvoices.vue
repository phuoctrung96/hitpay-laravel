<template>
    <div>
        <div class="card border-0 shadow-sm mb-3">
            <div class="card-body">
                <h2 class="text-primary mb-0 title">Create Invoices In Bulk</h2>
            </div>
            <div class="card-body border-top">
                <p>
                    Download the invoice feed template :
                    <button
...
export default {
    ...
    methods: {
        ...
        {
            const fileName = window.HitPay.app_name + '-invoice-feed-template.csv';
            this.is_downloading = true;
            axios.get(this.getDomain('business/' + Business.id + '/invoice/download-feed-template', 'dashboard'),
                {
                    responseType: 'blob'
                }).then(({data}) => {
        ...
        uploadFeedTemplate()
        {
            ...
            this.is_busy = true;
            axios.post(this.getDomain('business/' + Business.id + '/invoice/upload-feed-file', 'dashboard'),
                formData,
                {
                    headers: {
