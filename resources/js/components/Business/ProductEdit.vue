...
<template>
  <div>
    <div class="card border-0 shadow-sm mb-3">
      <div class="card-body p-4">
        ...
        <div class="form-group">
          ...
          <input id="name" type="text" v-model="product.name" :class="{ 'is-invalid' : errors.name }"
                 class="form-control bg-light" title="Product Name" placeholder="T-Shirt">
          <span v-if="errors.name" class="invalid-feedback error d-block" role="alert">
            {{errors.name}}
          </span>
        </div>
        <div class="form-group">
          <label>SKU</label>
          <input id="stock_keeping_unit" type="text" v-model="product.stock_keeping_unit" :class="{ 'is-invalid' : errors.stock_keeping_unit }"
                 class="form-control bg-light" title="SKU" placeholder="SKU">
          <span v-if="errors.stock_keeping_unit" class="invalid-feedback" role="alert">
        ...
        <div class="form-group">
          ...
          <div class="input-group">
            ...
            <input id="price" v-model="product.price" :class="{ 'is-invalid' : errors.price }"
        ...
        <div class="form-group mb-0">
          ...
          <ckeditor id="description" v-model="product.description"
        ...
        <div v-if="product.is_manageable" class="mt-3">
          <div class="form-row">
            <div class="form-group col-12 col-sm-6 mb-sm-0">
              ...
              <input v-model="product.quantity" :class="{ 'is-invalid' : errors.quantity }"
              ...
              <span v-if="errors.quantity" class="invalid-feedback"
                    role="alert">{{ errors.quantity }}</span>
            </div>
    ...
    <div class="card border-0 shadow-sm mb-3" v-if="product.variations_count > 1">
      ...
      <table v-if="product.variations.length > 0" class="table table-hover border-bottom mb-0">
        ...
        <template v-for="(item, key) in product.variations">
          <tr :class="{'alert-danger': (errors.variations && errors.variations[item.id]) }">
            <td>
            ...
            <td>
              <div class="input-group">
                ...
                <input :id="`variation.${key}.price`" v-model="item.price" title="Selling Price" class="form-control"
                       :class="{'is-invalid' : hasErrorKey(`variation.${key}.price`) }"
                       :placeholder="product.price" step="0.01">
                <span v-if="hasErrorKey(`variation.${key}.price`)" class="invalid-feedback" role="alert">
                  {{ getErrorKey(`variation.${key}.price`) }}
                </span>
              </div>
            </td>
            <td v-if="product.is_manageable">
              <input :id="`variation.${key}.quantity`" v-model="item.quantity" title="Quantity"
                     class="form-control"
                     :class="{'is-invalid' : hasErrorKey(`variation.${key}.quantity`) }"
                     @keydown="disallowDecimal($event)">
              <span v-if="hasErrorKey(`variation.${key}.quantity`)" class="invalid-feedback" role="alert">
                  {{ getErrorKey(`variation.${key}.quantity`) }}
              </span>
            </td>
            <td v-if="product.is_manageable">
              <input :id="`variation.${key}.quantity_alert_level`" v-model="item.quantity_alert_level" title="Quantity"
                     class="form-control"
                     :class="{'is-invalid' : hasErrorKey(`variation.${key}.quantity_alert_level`) }"
                     :disabled="item.quantity === ''" @keydown="disallowDecimal($event)">
              <span v-if="hasErrorKey(`variation.${key}.quantity_alert_level`)" class="invalid-feedback" role="alert">
                  {{ getErrorKey(`variation.${key}.quantity_alert_level`) }}
              </span>
            </td>
          </tr>
          <tr v-if="errors.variations && errors.variations[item.id]">
          ...
          <tr :class="{
            ...
            <td>
              <div class="input-group">
                ...
                <input :id="`new_variation.${key}.price`" v-model="item.price" title="Selling Price"
                       :class="{'is-invalid' : hasErrorKey(`new_variation.${key}.price`) }"
                       class="form-control" :placeholder="product.price" step="0.01">
                <span v-if="hasErrorKey(`new_variation.${key}.price`)" class="invalid-feedback" role="alert">
                  {{ getErrorKey(`new_variation.${key}.price`) }}
                </span>
              </div>
            </td>
            <td v-if="form_control.is_variation_manageable">
              <input :id="`new_variation.${key}.quantity`" v-model="item.quantity" title="Quantity"
                     :class="{'is-invalid' : hasErrorKey(`new_variation.${key}.quantity`) }"
                     class="form-control"
                     @keydown="disallowDecimal($event)">
              <span v-if="hasErrorKey(`new_variation.${key}.quantity`)" class="invalid-feedback" role="alert">
                {{ getErrorKey(`new_variation.${key}.quantity`) }}
              </span>
            </td>
            <td v-if="form_control.is_variation_manageable">
              <input :id="`new_variation.${key}.quantity_alert_level`" v-model="item.quantity_alert_level" title="Quantity" class="form-control"
                     :class="{'is-invalid' : hasErrorKey(`new_variation.${key}.quantity_alert_level`) }"
                     :disabled="item.quantity === null || item.quantity === ''"
                     @keydown="disallowDecimal($event)">
              <span v-if="hasErrorKey(`new_variation.${key}.quantity_alert_level`)" class="invalid-feedback" role="alert">
                {{ getErrorKey(`new_variation.${key}.quantity_alert_level`) }}
              </span>
            </td>
          </tr>
    ...
    <p class="text-danger font-weight-bold text-center mb-0" v-if="Object.keys(errors).length > 0">
      Scroll up to review required fields to proceed
    </p>
          ...
          <div class="modal-body" v-if="!is_loading">
            <div class="form-group" v-for="variation in product.variation_types">
              ...
              <input type="text" :title="usable_data.variation_list[variation]"
                     ...
                     :class="{ 'is-invalid' : (errors.temp_variation && errors.temp_variation[variation]) }">
              <span v-if="(errors.temp_variation && errors.temp_variation[variation])" class="invalid-feedback"
                    role="alert">{{ (errors.temp_variation && errors.temp_variation[variation]) }}</span>
            </div>
              ...
              <div class="input-group">
                <div class="input-group-prepend">
                                    <span class="input-group-text"
                                          :class="{ 'border-danger' : (errors.temp_variation && errors.temp_variation.price) }">{{
                                            display_currency
                                        }}</span>
                </div>
                <input title="Selling Price" class="form-control bg-light" :placeholder="product.price"
                       ...
                       :class="{ 'is-invalid' : errors.temp_variation && errors.temp_variation.price }">
              </div>
              <span v-if="errors.temp_variation && errors.temp_variation.price" class="small text-danger"
                    role="alert">{{ errors.temp_variation && errors.temp_variation.price }}</span>
            </div>
              ...
              <div class="form-row mb-0">
                <div class="form-group col-12 col-sm-6 mb-sm-0">
                  ...
                  <input v-model="control.temp_variation.quantity"
                         :class="{ 'is-invalid' : errors.temp_variation && errors.temp_variation.quantity }"
                         class="form-control bg-light" title="Available Quantity" placeholder="10"
                         @keydown="disallowDecimal($event)">
                  <span v-if="errors.temp_variation && errors.temp_variation.quantity" class="invalid-feedback"
                        role="alert">{{ errors.temp_variation && errors.temp_variation.quantity }}</span>
                </div>
                <div class="form-group col-12 col-sm-6 mb-0">
                  <label>Low Quantity Alert</label>
                  <input v-model="control.temp_variation.quantity_alert_level" :class="[
                                        {
                                            'is-invalid' : errors.temp_variation && errors.temp_variation.quantity_alert_level
                                        },
                                        {
                         ...
                         @keydown="disallowDecimal($event)">
                  <span v-if="errors.temp_variation && errors.temp_variation.quantity_alert_level" class="invalid-feedback"
                        role="alert">{{ errors.temp_variation && errors.temp_variation.quantity_alert_level }}</span>
                </div>
              </div>
            </div>
          </div>
...
export default {
      ...
      handler(values) {
        ...
        if (values.quantity) {
          ...
          if (values.quantity_alert_level) {
            if (typeof values.quantity_alert_level === 'string') {
  ...
  mounted() {
    ...
    _.each(this.product.variations, (value, index) => {
      ...
      this.product.variations[index]['quantity_alert_level'] = (value.quantity_alert_level === null || value.quantity_alert_level === "") ? '' : value.quantity_alert_level;
    });
  ...
  methods: {
    ...
    addVariation(event) {
      ...
      if (Object.keys(this.errors.temp_variation).length > 0) {
        ...
        delete this.errors.temp_variation;
      }
    ...
    updateProduct(publish) {
      ...
      if (this.product.description && this.product.description.length > 65536) {
        this.errors.description = 'The description may not be greater than 65,536 characters.';
      } else if (this.product.price < 1) {
        this.errors.price = 'The price can\'t be lower than $1.';
      } else if (this.product.price > 99999999999999) {
        this.errors.price = 'The price can\'t be greater than $99999999999999.';
      }

      if (this.product.stock_keeping_unit != null && this.product.stock_keeping_unit.length >= 255) {
        this.errors.stock_keeping_unit = 'The SKU may not be greater than 255 characters.';
      }

      if (this.product.variations_count <= 1 && this.product.is_manageable) {
        if (!this.product.quantity) {
          this.errors.quantity = 'The quantity field is required when the product inventory is tracked.';
        } else if (isNaN(this.product.quantity)) {
          this.errors.quantity = 'The quantity must be a number.';
        }

        if (this.product.quantity_alert_level && this.product.quantity_alert_level.length && isNaN(this.product.quantity_alert_level)) {
          this.errors.quantity_alert_level = 'The quantity alert must be a number.';
        }
      }
      ...
      if (this.product.new_variations && this.product.new_variations.length === 1){
        this.errors.variation = 'You can\'t create a product with just one variant' ;
      }
      if (this.product.new_variations && this.product.new_variations.length > 100){
        this.errors.variation = 'You can\'t create more than 100 variants' ;
      }

      // handle errors product
      ...
      // handle errors variations
      if (this.errors.variations && this.variation_error) {
        if (Object.keys(this.errors.variations).length > 0 || Object.keys(this.variation_errors).length > 0) {
          loading.remove();
      ...
      if (this.product.is_manageable && this.product.variations_count <= 1 && this.product.new_variations.length < 1) {
        ...
        _.forEach(this.product.variations, value => {
          ...
          if (this.product.is_manageable) {
            ...
            if (value.quantity_alert_level === '') {
              form.append('variation[' + i + '][quantity_alert_level]', "");
            } else {
        ...
        _.forEach(this.product.new_variations, (value) => {
          ...
          if (this.form_control.is_variation_manageable) {
            ...
            if (value.quantity_alert_level === '') {
              form.append('new_variation[' + i + '][quantity_alert_level]', "");
            } else {
    ...
    handleImage(event) {
      ...
      let i = (this.product.image).length;
