<template>
  <div class="shopee-page p-4 d-flex flex-column justify-content-center">
    <h2 class="text-primary mb-3">{{ zipConnected ? 'Connected to Zip' : 'Set up Zip' }}</h2>
    ...
    <div
      v-if="successMessage"
      class="w-100 alert alert-success">
      Zip setup has been completed. <a :href="integrationsLink" class="text-underline">Enable it now</a> in your checkout page.
    </div>
    ...
    <template v-if="zipConnected">
      ...
      <p class="font-weight-bold text-danger mt-4">Remove Zip Account</p>
      ...
      <button
        ...
        Remove Zip Account <i v-if="isProcessing" class="fas fa-circle-notch fa-spin"></i>
      </button>
    </template>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    integrationsLink () {
      return this.getDomain(`/business/${this.business.id}/gateway-provider`, 'dashboard')
    }
  },
  async mounted () {
    this.zipConnected = Boolean(this.provider)
    this.edit = !this.zipConnected

    if (this.zipConnected) {
  ...
  methods: {
    async onSave () {
      ...
      if (this.edit) {
        ...
        try {
          ...
          const res = await axios.post(this.getDomain(`business/${this.business.id}/payment-provider/zip/`, 'dashboard'),
            formData,
            {
              headers: {
    ...
    async onRemoveShopee () {
      ...
      try {
        ...
        await axios.post(this.getDomain(`business/${this.business.id}/payment-provider/zip/remove`, 'dashboard'), {
          password: this.form.password
        })

        this.zipConnected = false
        this.clearForm()
        this.edit = true
      } catch (error) {
        if (error.response.status === 422) {
