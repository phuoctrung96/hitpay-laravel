    ...
    <div class="row">
        <div class="col-md-9 col-lg-8 main-content">
            <div class="btn-group btn-group-sm mb-3 w-100 shadow-sm">
                <a class="btn col {{ $status === 'pending' ? 'active btn-outline-primary' : 'btn-light' }}" href="{{ route('admin.commission.index', [
                    'status' => 'pending',
                ]) }}">Pending</a>
                <a class="btn col {{ $status === 'succeeded' ? 'active btn-outline-primary' : 'bg-light' }}" href="{{ route('admin.commission.index', [
                    'status' => 'succeeded',
                ]) }}">Succeeded</a>
            </div>
            <div class="card border-0 shadow-sm mb-3">
                <div class="card-body p-4">
                    <h2 class="text-primary mb-0 title">Commissions</h2>
                </div>
                ...
                @if ($paginator->count())
                    @foreach ($paginator as $payout)
                        @if ($payout['status'] === 'request_pending')
                            <a class="hoverable" href="{{ route('admin.commission.get', [
                                $payout->getKey()
                            ]) }}">
                        @endif
                        <div class="card-body bg-light border-top p-4">
                            ...
                            @endif
                            @php($accountDetails = explode('@', $payout->payment_provider_account_id))
                            <p class="text-dark small mb-2">Payout Destination:
                                <span class="text-muted">
                                    {{ isset($accountDetails[0]) && isset(\App\Business\Transfer::$availableBankSwiftCodes[$accountDetails[0]]) ? \App\Business\Transfer::$availableBankSwiftCodes[$accountDetails[0]] : $accountDetails[0] }} ({{ $accountDetails[1] ?? 'unknown' }})
                                </span>
                            </p>
            ...
            <admin-business-commission-export></admin-business-commission-export>
        </div>
    </div>
@endsection
